...
__global__ void setBounderies(
	...
	if ( t_j==0 )
		writeBound( 1, phi,	bound_l, t_j, t_i, t_k, tid );
	if ( t_j==( N_W - 1) )
		writeBound( 1, phi, bound_r, t_j, t_i, t_k, tid );
	if ( t_i==0 )
		writeBound( 2, phi, bound_d, t_j, t_i, t_k, tid );
	if ( t_i==( N_H - 1 ) )
		writeBound( 2, phi,	bound_u, t_j, t_i, t_k, tid );
	// if ( t_k==0 )
	// 	writeBound( 3, phi, bound_b, t_j, t_i, t_k, tid );
	// if ( t_k==( N_D -1 ) )
	// 	writeBound( 3, phi,	bound_t, t_j, t_i, t_k, tid );
}
